...
import { LogLevel, type LogEntry, type PartialLogEntry } from '../../src/types';
import { generateLogId, now } from '../../src/utils/helpers';
...
describe('LogCollector', () => {
  ...
  describe('initialization', () => {
    test('should create collector with default options', () => {
      const defaultCollector = new LogCollector({ service: 'test' });
      expect(defaultCollector).toBeInstanceOf(LogCollector);
    ...
    test('should throw error for invalid service name', () => {
      expect(() => new LogCollector({ service: '' })).toThrow();
    });
  });
  ...
  describe('collect', () => {
    ...
    test('should enrich log entry when enabled', async () => {
      const enrichedCollector = new LogCollector({
        service: 'test',
  ...
  describe('collectMany', () => {
    test('should collect multiple log entries', async () => {
      const entries: PartialLogEntry[] = [
        { message: 'Log 1', level: LogLevel.INFO, service: 'test' },
        { message: 'Log 2', level: LogLevel.WARN, service: 'test' },
        { message: 'Log 3', level: LogLevel.ERROR, service: 'test' },
      ];
  ...
  describe('flush', () => {
    test('should flush buffer when batch size is reached', async () => {
      ...
      const entries: PartialLogEntry[] = Array.from({ length: 10 }, (_, i) => ({
        message: `Log ${i}`,
        level: LogLevel.INFO,
        service: 'test',
      }));
  ...
  describe('child', () => {
    test('should create child collector', () => {
      ...
      expect(child).toBeInstanceOf(LogCollector);
    });
